...
module.exports.getEsLintFiles = (folder, ignore) =>
  globSync('**/*.{js,json,md}', {
    cwd: folder,
    ignore,
    dot: true
...
module.exports.getTestFiles = folder =>
  globSync('test/**/*.spec.js', {
    cwd: folder,
    ignore: ['**/node_modules/**', '**/coverage/**', '**/flow-typed/**'],
    dot: true
  });

module.exports.getYamlFiles = folder =>
  globSync('**/*.{yml,yaml}', {
    cwd: folder,
    ignore: ['**/node_modules/**'],
    dot: true
...
const getRepoUrl = (cwd, remote) => {
  const url = String(spawnSync('git', ['config', '--get', `remote.${remote}.url`], { cwd }).stdout).trim();
  const parsed = gh(url);
...
module.exports.readListFile = filePath =>
...
module.exports.loadConfig = (cwd, name) =>
  this.readTextFile(`${__dirname}/conf/${name}`)
    .split('\n')

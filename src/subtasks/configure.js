...
const updateStruct = (cwd, files, converter) =>
  files.forEach(fileName => {
    const filePath = path.join(cwd, fileName);
    const expected = converter[0](
      fs.readFileSync(path.join(__dirname, '..', 'templates', 'merge', `dot${fileName}`), 'utf8')
    );
...
const updateSequential = (cwd, folders) =>
...
module.exports = (logger, cwd, config) => {
  ...
  const filteredTasks = Object.keys(tasks).reduce(
    (p, c) => Object.assign(p, { [c]: tasks[c].filter(f => toSkip.indexOf(f) === -1) }),
    {}
  );

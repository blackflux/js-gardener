...
module.exports = (grunt) => {
  ...
  grunt.registerMultiTask('gardener', function () {
    ...
    grunt.registerTask('gardener_configure', () => {
      ...
      const badges = grunt.file.readJSON(`${__dirname}/templates/badges.json`);
      const readmeFile = path.join(options.root, 'README.md');
      const readmeMd = grunt.file.read(readmeFile);
      const prepend = badges.filter(badge => !readmeMd
        .match(new RegExp(badge
          .replace(/[-[\]/{}()*+?.\\^$|]/g, "\\$&")

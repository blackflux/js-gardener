...
describe('Testing struct', () => {
  ...
  it('Testing Success', (done) => {
    struct(logger, projectFolder, ['test/mock/test/index.spec.js']).then(() => {
  ...
  it('Testing Failure', (done) => {
    struct(logger, projectFolder, []).catch((r) => {
      ...
      expect(logs[0], `Provided ${logs}`).to.contain('index.js to exist for ');
      expect(logs[0], `Provided ${logs}`).to.contain('index.spec.js');
      done();
    });
  });
});
